				...
				<div class="lrp" id="thrive_container_form_add_comment"
					...
					<form action="<?php echo site_url( '/wp-comments-post.php' ) ?>" method="post"
						...
						<?php if ( ! is_user_logged_in() ): ?>
							<input type="text" placeholder="<?php _e( "Name", "thrive" ) ?><?php if ( get_option( "require_name_email" ) == 1 ) { ?>*<?php } ?>" id="author" author="author" class="text_field author" name="author"/>
							<input type="text" placeholder="<?php _e( "Email", "thrive" ) ?><?php if ( get_option( "require_name_email" ) == 1 ) { ?>*<?php } ?>" id="email" author="email" class="text_field email" name="email"/>
						 ...
						 * For Wp Recipe Maker
						 *
						 * This filter is called by the plugin for the rating functionality
						 */
						apply_filters( 'comment_form_logged_in_after', '' );
						?>
