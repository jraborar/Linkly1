...
class MyTheme_Customize {
	...
	public static function generate_fonts_includes() {
		...
		if ( $header_font ) {
			...
			} else if ( strpos( $header_font, "=" ) === false && strlen( $header_font ) > 1 ) {  //hot fix for font manager changes
				$header_font = '';
			}

			echo empty( $header_font ) ? '' : "<link href='" . $header_font . "' rel='stylesheet' type='text/css'>";
		}
		if ( $body_font ) {
			if ( Tve_Dash_Font_Import_Manager::isImportedFont( $body_font ) ) {
				$body_font = Tve_Dash_Font_Import_Manager::getCssFile();
			} else if ( strpos( $body_font, "=" ) === false && strlen( $body_font ) > 1 ) {
				$body_font = '';
			}

			echo empty( $body_font ) ? '' : "<link href='" . $body_font . "' rel='stylesheet' type='text/css'>";
		}
	}
