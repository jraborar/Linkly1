...
function thrive_meta_apprentice_lesson_options( $post ) {
	...
	//Backwards compatibility for previous json encoded links
	if ( $downloadLinksJson && maybe_unserialize( $downloadLinksJson ) ) {
		$downloadLinksArray = maybe_unserialize( $downloadLinksJson );

		if ( $downloadLinksArray && is_array( $downloadLinksArray ) ) {
			$new_links = array();
			foreach ( $downloadLinksArray as $piece ) {
				$new_links[] = (array) $piece;
			}

			$downloadLinksArray = $new_links;
		}
...
function _thrive_save_apprentice_lesson_options( $post_data ) {
	...
	if ( $apprCategory > 0 ) {
		...
		if ( isset( $lessonsOrder[ $apprCategory ] ) ) {
			$lessonsOrder[ $apprCategory ] = is_array( $lessonsOrder[ $apprCategory ] ) ? $lessonsOrder[ $apprCategory ] : array();
			$hasCategory                   = in_array( $post_id, $lessonsOrder[ $apprCategory ] );
		}

		if ( $apprCategory != $previousCatId || ! $hasCategory ) {
	...
	$thrive_meta_appr_download_links_json = json_decode( $thrive_meta_appr_download_links_json );
	$thrive_meta_appr_download_links_json = serialize( $thrive_meta_appr_download_links_json );
