...
function thrive_get_post_date( $date_type = 'publish', $relative_time = 1 ) {
	if ( $date_type == 'publish' || get_the_modified_date() == get_the_date() ) {
		...
		if ( $relative_time == 1 ) {
			$post_date = __( 'Updated', 'thrive' ) . ' ' . thrive_human_time( get_the_modified_time( 'U' ) );
		} else {
			$post_date = __( 'Updated', 'thrive' ) . ' ' . get_the_modified_date();
		}
	}
	...
	if ( is_single() || is_page() ) {
		...
		if ( ! isset( $theme_options['social_site_image'] ) || $theme_options['social_site_image'] == '' ) {
			if ( has_post_thumbnail( get_the_ID() ) ) {
				...
				if ( $featured_image ) {
					$theme_options['social_site_image'] = $featured_image;
				}
			}
		}
