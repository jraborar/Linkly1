...
function woo_mobile_menu_item( $items, $args ) {
	...
	/**
	 * Do not automatically include WC link if rendering TAr menu (first part of the condition).
	 * The second part is a mystery to me - retarded copy-pasted code most likely. I've left it "as is" due to backwards compat issues...
	 */
	if ( ! isset( $GLOBALS['tcb_wp_menu'] ) && ( $args->theme_location == 'primary' || class_exists( 'WooCommerce' ) ) ) {
		$item = '<a href="' . wc_get_cart_url() . '">';
		$item .= sprintf( _n( '%d - item', '%d - items', WC()->cart->cart_contents_count, 'thrive' ), WC()->cart->cart_contents_count );
		$item .= '</a>';
