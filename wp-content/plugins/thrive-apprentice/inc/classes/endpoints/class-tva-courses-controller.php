...
class TVA_Courses_Controller extends TVA_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route( self::$namespace . self::$version, '/' . $this->base . '/search_users', array(
			array(
				'methods'             => WP_REST_Server::READABLE,
		...
		register_rest_route( self::$namespace . self::$version, '/' . $this->base . '/bulk_action', array(
			array(
				'methods'             => WP_REST_Server::EDITABLE,
		...
		register_rest_route( self::$namespace . self::$version, '/' . $this->base . '/update_posts_order', array(
			array(
				'methods'             => WP_REST_Server::EDITABLE,
	...
	public function update_term_meta() {
		...
		if ( TVA_Const::TVA_IS_COURSE_COMMENT_STATUS_CHANGED !== $this->request->get_param( 'comment_status_changed' ) ) {
			...
			if ( isset( $ids ) && is_array( $ids ) ) {
				TVA_Lessons_Controller::update_lesons_comment_status( $ids, $this->request->get_param( 'comment_status' ) );
			}
		}
	...
	public function search_users( $request ) {
		...
		foreach ( $users as $user ) {
			$result[] = array(
				'id'    => $user->data->ID,
				'type'  => $user->data->user_email,
				'label' => $user->data->display_name,
				'url'   => get_avatar_url( $user->data->ID ),
			);
		}
	...
	public function handle_bulk_action( $request ) {
		if ( ! self::is_valid_request( $request ) ) {
			return new WP_Error( 'invalid-request', __( 'Invalid Request. Please reload the page', TVA_Const::T ) );
		}
		...
		if ( $action === 'move' ) {
			...
			$result     = $this->$fn( $data, $high_level );
		} else {
			$result = $this->$fn( $data );
		}
	...
	public function delete_bulk_items( $items ) {
		...
		return tva_get_course_by_id( $this->term_id );
	}
	 ...
	 * @return WP_REST_Response
	 */
	public function move_bulk_items( $items, $high_level ) {
		...
		foreach ( $items as $item ) {
			try {
				...
				if ( $item['old_post_parent'] ) { //it might be zero
					$old_parents[] = $item['old_post_parent'];
				}

				if ( $high_level === 'module' ) { //modules

					$siblings = TVA_Manager::get_course_modules( $course_term );
					$tva_item = new TVA_Module( $item );
					$order    = count( $siblings );

				} elseif ( $high_level === 'chapter' ) { //chapters

					$tva_item = new TVA_Chapter( $item );

					if ( $tva_item->post_parent ) { //we move this into a new module
						$module   = get_post( $tva_item->post_parent );
						$siblings = TVA_Manager::get_module_chapters( $module );
						$order    = $module->tva_module_order . count( $siblings );
					} else {
						$siblings = TVA_Manager::get_course_chapters( $course_term );
						$order    = count( $siblings );
					}
				} else { //lessons

					$tva_item = new TVA_Lesson( $item );

					if ( $tva_item->post_parent ) {
						$parent        = get_post( $tva_item->post_parent );
						$new_parents[] = $tva_item->post_parent;
						$siblings      = TVA_Manager::get_children( $parent );
						$order         = $parent->post_type === TVA_Const::MODULE_POST_TYPE ? $parent->tva_module_order : $parent->tva_chapter_order;
						$order         = $order . count( $siblings );
					} else {
						$siblings = TVA_Manager::get_course_lessons( $course_term );
						$order    = count( $siblings );
					}
				}
		...
		foreach ( $new_parents as $new_parent_id ) {
			TVA_Manager::review_status( $new_parent_id );
		}
