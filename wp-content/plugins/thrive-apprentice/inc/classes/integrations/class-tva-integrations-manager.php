...
class TVA_Integrations_Manager {
	...
	private function _init_wishlist() {
		...
		$class_name = class_exists( 'WishListMember_Level', false ) ? 'TVA_WL_Integration' : 'TVA_Unknown_Integration';

		return new $class_name( 'wishlist', 'WishList Membership' );
	}
	...
	public function admin_localize( $data ) {
		...
		foreach ( $this->_integrations_instances as $integration ) {
			 ...
			 */
			if ( $integration instanceof TVA_Unknown_Integration ) {
				continue;
			}
