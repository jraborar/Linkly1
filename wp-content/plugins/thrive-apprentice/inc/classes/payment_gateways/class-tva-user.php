...
class TVA_User {
	 ...
	 */
	public static function get_sendowl_customers( $args = array() ) {
		global $wpdb;

		$placeholders = array();

		$args['limit']  = isset( $args['limit'] ) ? $args['limit'] : TVA_Const::SENDOWL_CUSTOMERS_PER_PAGE;
		...
		$select     = 'SELECT users.ID, users.display_name, users.user_email, users.user_login ';
		$from       = 'FROM ' . $tbl_users;
		$inner_join = 'INNER JOIN ' . $tbl_orders . ' ON users.ID = orders.user_id ';
		$left_join  = '';
		$where      = 'WHERE 1=1';

		$where          .= ' AND orders.`status` = %d';
		$placeholders[] = TVA_Const::STATUS_COMPLETED;

		$order = ' ORDER BY users.user_registered DESC ';
		...
		$sql     = $select . $from . $inner_join . $left_join . $where . $group . $order . $limit;
		$sql     = $wpdb->prepare( $sql, $placeholders );
		$results = $wpdb->get_results( $sql, ARRAY_A );

		return tva_parse_sendowl_customers( $results );
	}
	 ...
	 * @return array|object|null
	 */
	public static function get_customer_access_rights( $customer_id = 0 ) {
		global $wpdb;
		...
		$select     = 'SELECT items.* FROM ' . $items_table . ' AS items ';
		$inner_join = ' INNER JOIN ' . $orders_table . ' orders ON items.order_id = orders.ID ';

		$where        = ' WHERE orders.user_id = %d AND orders.status = %d';
		$placeholders = array( $customer_id, TVA_Const::STATUS_COMPLETED );

		$group = ' GROUP BY product_id ';

		$sql = $select . $inner_join . $where . $group;

		return $wpdb->get_results( $wpdb->prepare( $sql, $placeholders ), ARRAY_A );
	}
	...
	public function has_bought( $product_id ) {
		...
		foreach ( $this->get_orders_by_status( TVA_Const::STATUS_COMPLETED ) as $order ) {
			if ( true === $order instanceof TVA_Order && true === ( $order->get_order_item_by_product_id( $product_id ) ) instanceof TVA_Order_Item ) {
				$has_bought = true;
				break;
			}
		}
