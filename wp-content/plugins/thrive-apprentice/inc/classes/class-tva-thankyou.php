...
class TVA_Thankyou {
	...
	public function get_redirect_url( $product_id = 0 ) {
		$settings     = TVA_Sendowl_Settings::instance()->get_settings();
		$index_url    = TVA_Settings::get_index_page_url();
		...
		switch ( $settings['thankyou_page_type'] ) {
			case 'static':
				$_url         = get_permalink( $settings['thankyou_page']['ID'] );
				$redirect_url = $_url ? $_url : $redirect_url;
				break;

			case 'redirect':
				$redirect_url = $this->_get_redirect_url( $settings, $product_id );
	 ...
	 * @return string
	 */
	private function _get_redirect_url( $settings, $product_id ) {
		...
		$redirect_url = TVA_Settings::get_index_page_url();
		...
		if ( count( $product->protected_terms ) > 1 ) {
			...
			if ( ! empty( $settings['thankyou_multiple_page']['ID'] ) ) {
				$_url         = get_permalink( $settings['thankyou_multiple_page']['ID'] );
				$redirect_url = false === $_url ? TVA_Settings::get_index_page_url() : $_url;
			}
		...
		if ( $term instanceof WP_Error ) {
			return TVA_Settings::get_index_page_url();
		}
		...
		return $redirect_url instanceof WP_Error ? TVA_Settings::get_index_page_url() : $redirect_url;
	}
}
