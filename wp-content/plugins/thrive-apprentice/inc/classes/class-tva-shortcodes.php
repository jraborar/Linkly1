...
class TVA_Shortcodes {
	...
	public function tva_progress_bar( $atts, $content = '' ) {
		...
		if ( tva_is_course_guide( $this->course ) && ( isset( $obj ) && (int) $this->settings['index_page']['ID'] !== (int) $obj->ID ) ) {
			return $content;
		}
		...
		/**
		 * When doing ajax we don't have the post set so we need to get the ID from the referer
		 */
		if ( ! $post ) {
			$url     = wp_get_referer();
		 ...
		 */
		if ( ! empty( $this->settings ) && isset( $this->settings['template']['progress_bar'] ) && ( is_tax( TVA_Const::COURSE_TAXONOMY ) || ( isset( $this->settings['index_page']['ID'] ) && $post_id == $this->settings['index_page']['ID'] ) ) ) {
	 ...
	 * @return string
	 */
	public function tva_author( $atts, $content = '' ) {
		...
		if ( is_tax( TVA_Const::COURSE_TAXONOMY ) && ! empty( $this->settings ) && isset( $this->settings['template']['author'] ) ) {
			$title = $this->settings['template']['author'];
		} else {
			$title = isset( $atts['title'] ) ? $atts['title'] : TVA_Const::TVA_SHORTCODE_AUTHOR;
		}

		$author = $this->course ? $this->course->author : array();

		if ( empty( $author ) ) {
			$author['ID'] = get_current_user_id();
		}

		$custom_bio = false;

		if ( $this->course ) {
			$custom_bio = isset( $author['biography_type'] ) && 'custom_bio' === $author['biography_type'];
		}

		$bio       = true === $custom_bio ? $author['custom_biography'] : get_the_author_meta( 'description', $author['ID'] );
		$avatar    = ! empty( $author['url'] ) ? $author['url'] : get_avatar_url( $author['ID'] );
	...
	public function tva_render_lesson( $lesson ) {
		...
		if ( ! $allowed ) {
			$labels = tva_get_labels( array( 'ID' => $this->course->label ) );
			$view   = ( isset( $labels[0]['title'] ) && $labels[0]['title'] ) ? $labels[0]['title'] : __( 'Members Only' );
	...
	public function tva_checkout_link( $atts, $content = '' ) {
		$checkout_url = TVA_Settings::instance()->get_checkout_page_url();
		...
		$product  = isset( $atts['product'] ) ? '&pid=' . $atts['product'] : '&bid=' . $atts['bundle'];
		$discount = isset( $atts['thrv_so_discount'] ) ? '&thrv_so_discount=' . $atts['thrv_so_discount'] : '';
		$content  = '<a href="' . $checkout_url . '?pp=' . $atts['pp'] . $product . $discount . '">' . $atts['title'] . '</a>';
