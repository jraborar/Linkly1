...
if ( ! class_exists( 'TVD_Smart_Site' ) ) :
	...
	final class TVD_Smart_Site {
		...
		public function localize() {
			return array(
				...
				'data'   => array(
					...
					'fieldTypes' => TVD_Smart_DB::fieldTypes(),
				),
			);
		}
		 ...
		 * @return mixed
		 */
		public function smart_site_links_shortcodes( $links ) {
			if ( ! empty( $_REQUEST[ TVE_FRAME_FLAG ] ) ) {
				$groups           = $this->db->get_groups( 0, false );
				$globalFieldLinks = array();
				foreach ( $groups as $group ) {
					$fields = $this->db->getFieldsByType( $group['id'], TVD_Smart_DB::$types['link'] );
					if ( ! empty( $fields ) ) {
						$globalFieldLinks[ $group['name'] ] = array();
						foreach ( $fields as $globalfield ) {
							$data                                 = maybe_unserialize( $globalfield['data'] );
							$globalFieldLinks[ $group['name'] ][] = array(
					...
					if ( empty( $globalFieldLinks[ $group['name'] ] ) ) {
						unset( $globalFieldLinks[ $group['name'] ] );
					}
				}
				if ( ! empty( $globalFieldLinks ) ) {
					$links['Global Fields'] = array( 'links' => $globalFieldLinks, 'shortcode' => TVD_Smart_Site::GLOBAL_FIELDS_SHORTCODE_URL );
				}
			}
