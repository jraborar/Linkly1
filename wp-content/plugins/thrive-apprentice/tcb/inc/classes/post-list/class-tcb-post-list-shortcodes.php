...
class TCB_Post_List_Shortcodes {
	 ...
	 * @return string
	 */
	public static function custom_field( $attr = array(), $replacement = '', $is_text = true ) {
		...
		$content         = get_post_meta( $attr['post_id'], $attr['id'], true );
		$content         = isset( $content ) ? $content : $replacement;
		$content         = $is_text ? '<span>' . $content . '</span>' : $content;
		unset( $attr['post_id'] );
	...
	public static function post_date( $attr = array() ) {
		...
		$date = $attr['type'] === 'published' ? get_the_time( $format ) : get_the_modified_date( $format );

		$content = tcb_template( 'post-list-sub-elements/post-date.php', array(
			'url'  => empty( $attr['url'] ) ? 0 : $attr['url'],
			'date' => $date,
		), true );
	...
	private static function replace_postlist_dynamic_link( $content ) {
		...
		while ( preg_match( '(\[thrive_postlist_custom_fields_shortcode_url id=(.*?)\])', $content, $aux ) ) {
			$replacement = TCB_Post_List_Shortcodes::custom_field( array( 'id' => $aux[1] ), '', false );
