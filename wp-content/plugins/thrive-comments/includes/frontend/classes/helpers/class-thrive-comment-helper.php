...
class Thrive_Comments_Helper {
	...
	public function get_comments_from_post( $query_comments ) {
		...
		if ( empty( $parent_found ) ) {
			...
			do {
				// Take another page of comments if the user is unable to see the first pageor if the page is incomplete.
				$comments                   = get_comments( $query_comments );
				$filtered_comments          = $this->filter_comments( $comments );
	...
	public function parse_comment( $comment, $level ) {
		...
		$avatar      = get_comment_meta( $comment->comment_ID, 'comment_author_picture', true );
		if ( $avatar ) {
			$comment_aux['photo_src']     = $avatar;
			$comment_aux['social_avatar'] = 'true';
		} else {
			$comment_aux['photo_src'] = $this->get_picture_url();
		}
