...
abstract class TCB_Symbol_Element_Abstract extends TCB_Cloud_Template_Element_Abstract {
	...
	public static function get_default_thumb_placeholder() {
		return array(
			'url' => tcb_admin()->admin_url( 'assets/images/no-template-preview.jpg' ),
		...
		if ( isset( $cloud_headers[0] ) ) {
			foreach ( $cloud_headers as $cloud_header ) {
				$result[ $cloud_header['id'] ] = array(
					...
					'thumb'      => array(
						'url' => $cloud_header['thumb'],
						'w'   => isset( $cloud_header['thumb_size'] ) ? $cloud_header['thumb_size']['w'] : '',
						'h'   => isset( $cloud_header['thumb_size'] ) ? $cloud_header['thumb_size']['h'] : '',
					),
					'from_cloud' => 1,
				);
			}
