 ...
 */
add_action( 'tcb_settings_links', 'tve_ult_templates_setup_menu_items', 10, 0 );
...
function tve_ult_disable_controls( $controls ){
	$post_type = get_post_type();
	if ( tve_ult_is_editable( $post_type ) ) {
		$controls['countdown']['disabled_controls'] = array( '.tve-control:not([data-view="Color"])', '.control-grid:not(.color-picker-control):not(.color-picker-input)');
	}
	return $controls;
}
