...
( function ( $ ) {
	...
	$( function () {
		...
		ThriveUlt.views.ModalEditDateSettings = TVE_Dash.views.Modal.extend( {
			...
			afterOpen: function () {
				this.$( '#tvu-timezone-setting' ).val( this.model.get( 'timezone' ) );
			},
			...
			changeTimeZone: function ( e ) {
				this.model.set( 'timezone', e.target.value );
			},
			save: function () {
				this.tvd_clear_errors();
				...
				this.model.save().done( function ( r ) {
					TVE_Dash.hideLoader();
					ThriveUlt.data.now = r.now;
					ThriveUlt.data.settings.timezone = self.model.get( 'timezone' );
				} ).error( function () {
					//todo: error handling
					TVE_Dash.hideLoader();
				} ).always( function () {
...
( function ( $ ) {
	...
	$( function () {
		...
		Backbone.View.prototype.refreshTime = function () {
			this.$( '.tu-time' ).html( ThriveUlt.data.now );
			this.$( '.tu-timezone' ).html( ThriveUlt.data.settings.timezone );
		};
