...
if ( ! class_exists( 'TCB_Editor_Ajax' ) ) {
	...
	class TCB_Editor_Ajax {
		...
		public function action_lazy_load() {
			...
			$is_landing_page = tcb_editor()->is_landing_page();
			...
			if ( tcb_editor()->can_use_landing_pages() ) {
				...
				if ( $is_landing_page ) {
					...
					$data['lp_blocks'] = tcb_editor()->elements->get( 'contentblock' )->get_lp_cloud_templates( array(
						'nocache' => false,
						'lp_set'  => $landing_page->set,
					) );
				}
			}
		...
		public function action_cloud_content_templates_without_element() {
			...
			$templates = tve_get_cloud_content_templates( $type );
		 ...
		 */
		public function action_distraction_free() {
			$is_on   = $this->param( 'is_on', '0' );
			$user_id = get_current_user_id();
			update_user_meta( $user_id, 'distraction_free', $is_on );

			$this->json( get_user_meta( $user_id, 'distraction_free' ) );
		}
