...
class TCB_Post_List {
	...
	public static function post_info( $order = 0 ) {
		...
		if ( empty( $id ) ) {
			...
			$post = array(
				...
				'tcb_post_custom_fields_data'  => $custom_fields[0],
				'tcb_post_custom_fields_links' => $custom_fields[1],
			);
		}
	...
	public static function get_post_custom_fields( $id ) {
		$custom_keys = array_filter( (array) get_post_custom_keys( $id ), function ( $meta ) {
			return apply_filters( 'is_protected_meta', false, $meta, null ) === false;
		} );
		//Separate into 2 arrays: data and links
		$result = array( array(), array() );
		$custom = get_post_custom( $id );
		foreach ( $custom_keys as $val ) {
			$result[ filter_var( $custom[ $val ][0], FILTER_VALIDATE_URL ) ? '1' : '0' ][] = $val;
		}
